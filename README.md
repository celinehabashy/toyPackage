...
The goal of toyPackage is to make regular expressions more exciting! It
...
str_split_one <- function(x, pattern) {
    if (length(x) != 1) {
...
``` r
str_split_one <- function(x, pattern, n = Inf) {
    if (length(x) != 1) {
        stop("x must be a character vector of length 1")
    }
    unlist(stringr::str_split(x, pattern, n = n))
}
